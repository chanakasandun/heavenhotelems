...
export default function EligibleLeaveBalance() {
  ...
  const handleOpenEdit = (row) => {
    setLeaveEdit(row.leave_type)
    setLeaveBalance(row.balance)
  ...
  const filteredRows = rows.filter((row) =>
    row.leave_type.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const [leaveEdit, setLeaveEdit] = React.useState('');
        
  const handleLeaveChange = (event) => {
    setLeaveEdit(event.target.value);
  };

  const [leaveBalance, setLeaveBalance] = React.useState('');
        
  const handleLeaveBalanceChange = (event) => {
    setLeaveBalance(event.target.value);
  };
    ...
    <>
      ...
      <Box sx={{ display: "flex" }}>
        ...
        <Container>
          ...
          >
            <DialogTitle align="center" sx={{ fontSize: '18px'}}>Edit Employee Leave Balance</DialogTitle>
            <DialogContent>
              <TextField
                fullWidth
                label="Leave Type"
